...
abstract class PostFinanceCheckoutAbstractmigration
{
    ...
    protected static function updateCustomerIdOnTokenInfoBase()
    {
        ...
        $result = $instance->execute(
            ...
            "pfc_token_info` CHANGE `customer_id` `customer_id` int(10) unsigned NULL DEFAULT NULL;"
        );
        if ($result === false) {
            throw new Exception($instance->getMsgError());
        }
    ...
    protected static function updateImageBase()
    {
        ...
        $exists = $instance->executeS(
            "SHOW COLUMNS FROM `" . _DB_PREFIX_ . "pfc_method_configuration` LIKE 'image_base'"
        );
        if (empty($exists)) {
            $result = $instance->execute(
                "ALTER TABLE `" . _DB_PREFIX_ .
                "pfc_method_configuration` ADD COLUMN `image_base` varchar(2047) COLLATE utf8_unicode_ci NULL DEFAULT NULL AFTER image;"
            );
            if ($result === false) {
                throw new Exception($instance->getMsgError());
            }
        ...
        if (empty($exists)) {
            $result = $instance->execute(
                "ALTER TABLE `" . _DB_PREFIX_ .
                "pfc_transaction_info` ADD COLUMN `image_base` VARCHAR(2047)  COLLATE utf8_unicode_ci NULL DEFAULT NULL AFTER image;"
            );
            if ($result === false) {
                throw new Exception($instance->getMsgError());
            }
    ...
    protected static function userFailureMessageBase()
    {
        $instance = Db::getInstance();
        $exists = $instance->executeS(
            "SHOW COLUMNS FROM `" . _DB_PREFIX_ . "pfc_transaction_info` LIKE 'user_failure_message'"
        );
        if (empty($exists)) {
            $result = $instance->execute(
                "ALTER TABLE `" . _DB_PREFIX_ .
                "pfc_transaction_info` ADD COLUMN `user_failure_message` VARCHAR(2047)  COLLATE utf8_unicode_ci NULL DEFAULT NULL AFTER image;"
            );
            if ($result === false) {
                throw new Exception($instance->getMsgError());
            }
