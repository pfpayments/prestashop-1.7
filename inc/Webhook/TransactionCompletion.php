 ...
 */
class PostFinanceCheckout_Webhook_TransactionCompletion extends PostFinanceCheckout_Webhook_OrderRelatedAbstract {
    ...
    protected function loadEntity(PostFinanceCheckout_Webhook_Request $request){
		$completionService = new \PostFinanceCheckout\Sdk\Service\TransactionCompletionService(PostFinanceCheckout_Helper::getApiClient());
	...
	protected function update(\PostFinanceCheckout\Sdk\Model\TransactionCompletion $completion, Order $order, $success){
	    $completionJob = PostFinanceCheckout_Model_CompletionJob::loadByCompletionId($completion->getLinkedSpaceId(), $completion->getId());
	    if (!$completionJob->getId()) {
	        ...
	        $completionJob = PostFinanceCheckout_Model_CompletionJob::loadRunningCompletionForTransaction($completion->getLinkedSpaceId(),
	    ...
	    if($success){
	       $completionJob->setState(PostFinanceCheckout_Model_CompletionJob::STATE_SUCCESS);
	    }
	    else{
